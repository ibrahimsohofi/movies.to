...
export default function Register() {
  ...
  // Fetch trending movie for background
  useEffect(() => {
      ...
      try {
        ...
        if (data?.results?.length > 0) {
          ...
          if (moviesWithBackdrop.length > 0) {
            // Pick a different random movie (index 5-9) to differ from Login page
            // If less than 10 movies, pick from the last 5
            let start = Math.max(0, moviesWithBackdrop.length - 5);
            let end = moviesWithBackdrop.length;
            const randomIndex = Math.floor(Math.random() * (end - start)) + start;
            setBackgroundMovie(moviesWithBackdrop[randomIndex]);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (backgroundMovie?.backdrop_path) {
      const img = new Image();
      img.src = getImageUrl(backgroundMovie.backdrop_path, 'w1280');
  ...
  const backdropUrl = backgroundMovie?.backdrop_path
    ? getImageUrl(backgroundMovie.backdrop_path, 'w1280')
    : null;

  return (
    <div className="w-full py-8 md:py-12 flex items-center justify-center relative overflow-hidden">
      {/* Dynamic trending movie background */}
      <div
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-br from-gray-900 via-gray-800 to-black transition-opacity duration-1000 pointer-events-none ${
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-pink-500/20 via-transparent to-red-500/20 animate-gradient-shift pointer-events-none"></div>
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(236,72,153,0.15),transparent_50%)] pointer-events-none"></div>

      <Card className="w-full max-w-md mx-4 relative z-10 backdrop-blur-md bg-background/80 shadow-2xl border-pink-500/20 hover:border-pink-500/40 transition-all duration-500 animate-in fade-in slide-in-from-bottom-4">
