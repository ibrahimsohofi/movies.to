...
This project previously used SQLite. All SQLite code has been removed. The schema has been converted to MySQL with the following changes:

- `INTEGER` → `INT`
- `TEXT` → `VARCHAR()` or `TEXT`
- `REAL` → `DECIMAL()`
- `AUTOINCREMENT` → `AUTO_INCREMENT`
- Boolean values now use MySQL's `BOOLEAN` type
- Timestamps use MySQL's `TIMESTAMP` with automatic updates
